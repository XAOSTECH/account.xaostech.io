...
// OAuth callback proxy - redirect GitHub's callback from account subdomain
// This ensures cookies stay within same-site context
app.get('/auth/github/callback', async (c) => {
  ...
  // Forward to API worker's callback endpoint
  const apiCallbackUrl = new URL('/auth/github/callback', 'https://api.xaostech.io');
  ...
  if (!response.ok) {
    return new Response(JSON.stringify({ error: 'GitHub OAuth failed' }), { status: response.status });
  }
  
  // Get the session cookie from API and forward it
  const setCookieHeader = response.headers.get('set-cookie');
  const body = await response.text();
  
  const res = new Response(body, { status: response.status });
  if (setCookieHeader) {
