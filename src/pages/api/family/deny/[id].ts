...
export const POST: APIRoute = async ({ request, params, locals }) => {
    ...
    `).bind(contentId, session.user_id).first();
        ...
        // Deny the content - delete it
        if (pending.content_type === 'post') {
            await runtime.env.DB.prepare(
                'DELETE FROM posts WHERE id = ?'
            ).bind(pending.content_ref_id).run();
    ...
    `).bind(crypto.randomUUID(), session.user_id, pending.author_id, contentId, now).run();
        ...
        return new Response(null, {
            status: 302,
            headers: { 'Location': '/family?success=denied' }
        });

    } catch (err) {
        console.error('Deny content error:', err);
        return new Response(JSON.stringify({ error: 'Failed to deny content' }), {
            status: 500,
            headers: { 'Content-Type': 'application/json' }
        });
    }
